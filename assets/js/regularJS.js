...
var gameState = {
  ...
  pokemonDB: [
    ...
    defense: 43,
    level: 1,
    image: 'http://www.smogon.com/dex/media/sprites/xy/charmander.gif'
  }, {
    name: 'bulbasaur',
    type: 'grass',
    hp: 45,
    ...
    defense: 49,
    level: 1,
    image: 'http://www.smogon.com/dex/media/sprites/xy/bulbasaur.gif'
  }, {
    name: 'squirtle',
    type: 'water',
    hp: 44,
    ...
    defense: 65,
    level: 1,
    image: 'http://www.smogon.com/dex/media/sprites/xy/squirtle.gif'
  },
    ...
    init: function(){
      ...
      while (i < gameState.elements.pokemonsEl.length) {
        ...
        gameState.elements.pokemonsEl[i].onclick = function () {
          ...
          player1Img[0].src = gameState.currentPokemon[0].image

          // select data from current cpu pokemon
          gameState.currentRivalPokemon = gameState.pokemonDB.filter(function (pokemon) {
            return pokemon.name == gameState.rivalPokemon
          })
          player2Img[0].src = gameState.currentRivalPokemon[0].image

          // current user and cpu pokemon innitial health
...
console.log(gameState)
 ...
 checkWinner: function(enemy, attacker){
    if(enemy.health <= 0) {
      console.log('YOU ARE A WINNNERRRR!' + attacker.name)
  }
  },
 ...
 play: function(userAttack, cpuAttack) {
    ...
    switch (userAttack) {
      ...
      if (cpuAttack == 'paper') {
        ...
        if(currentRivalPokemon.health >= 1) {
        // cpu
        gameState.attackMove(currentRivalPokemon.attack, currentRivalPokemon.level, .8, 1, currentRivalPokemon, currentPokemon)
      }
  }
  }
      if (cpuAttack == 'scissors') {
      ...
      if (cpuAttack == 'paper') {
        ...
        if(currentRivalPokemon.health >= 1){
        // cpu
        gameState.attackMove(currentRivalPokemon.attack, currentRivalPokemon.level, .8, .5, currentRivalPokemon, currentPokemon)
      }
    }
  }
      if (cpuAttack == 'scissors') {
      ...
      if (cpuAttack == 'rock') {
        ...
        if(currentRivalPokemon.health >= 1) {
        // cpu
        gameState.attackMove(currentRivalPokemon.attack, currentRivalPokemon.level, .8, 2, currentRivalPokemon, currentPokemon)
  }
  }
      }
        break;
